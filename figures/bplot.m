...
wisColor = mapC(1,:);
scatColor = 0.6*[1 1 1];
meanColor = [0.8 0.1 0.05];

percentileNum = normcdf(-1)*100*2; % for the main quantiles
percentileNum2 = normcdf(-2)*100*2; % for the whisker ends
...
if horizontalFlag
    hReg2(end+1) = rectangle('Position',[boxEdge(1),y-barWidth/2,IQR,barWidth],'linewidth',linewidth,'EdgeColor',boxColor,'facecolor','none');
    ...
    hReg2(end+1) = rectangle('Position',[y-barWidth/2,boxEdge(1),barWidth,IQR],'linewidth',linewidth,'EdgeColor',boxColor,'facecolor','none');
    hold on;
