function y = logisticfit0(beta,x)
	...
	%	beta(3) = y0 (y-offset) [default 0]
	
	%define parameters
	if length(beta) < 1, L=1; else L = beta(1);end %asymptote
	if length(beta) < 2, k=1; else k = beta(2);end %slope
	if length(beta) < 3, y0=0; else y0 = beta(3);end %y-offset
	if length(beta) < 4, x0=0; else x0 = beta(4);end %x-offset
